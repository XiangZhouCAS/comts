...
option_list <- list(
  ...
  make_option(c("--singleM","-s"),type = "character",default = F,
              help = "please set the directory of singleM's universal single copy ribosomal genes (USCGs) database, but if you have already counted the RPKM of these genes, you can set the directory of result to skip it"),
  ...
  make_option(c("--run_seqkit","-k"),type = "character",default = "run",
	      help = "If you have already counted the number of all reads with seqkit, you can set the directory of seqkit result (e.g.'sample_name.all.reads.txt') to skip run seqkit, default is run seqkit"),
