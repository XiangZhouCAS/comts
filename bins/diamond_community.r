...
option_list <- list(
  ...
  make_option(c("--singleM","-s"),type = "character",default = F,
              help = "please set the directory of singleM's universal single copy ribosomal genes database, but if you have already counted the RPKM of these genes, you can set the directory of result to skip it"),
...
opt_parser = OptionParser(
  ...
  prog=NULL ,
  description = "This Script is to calculate single copy function enzyme gene community abandance.")
...
fastp <- sprintf("fastp -i %s -o %s --length_required 140 -w %d > /dev/null 2>&1",
...
if(run_fastp != "skip"){
  if(tmp$V1 != "text"){
    print("diamond is Running (Ribo maker genes).")
    system(diamond_singleM)
    print("diamond is completed (Ribo marker genes).")
...
hp <- ggplot(samplept,aes(sample,gene))+
  ...
  scale_fill_gradientn(colours = c("white","#C12554","#31357F"),
                       name = expression("Community [ Log"["2"]*"(%+1) ]"))+
...
ggsave(plot = hp,"Heatmap_community_abd.pdf",width = w*2,height = h/4)
